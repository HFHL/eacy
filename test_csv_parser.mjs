...
Papa.parse(content, {
  ...
  complete: function(results) {
    try {
      ...
      const shouyye = designModel.folders
         .find(f => f.name === '费用信息')
         ?.groups.find(g => g.name === '住院病案首页')
         ?.fields;
         
      console.log("\n住院病案首页 fields:");
      console.log(JSON.stringify(shouyye?.map(f => ({name: f.name, type: f.displayType, repeatable: f.repeatable, children: f.children?.length})), null, 2));
